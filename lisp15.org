;; This is gives an introduction to Emacs Lisp in 15 minutes
;;
...
;; `setq' store a value into a variable:
...
;; Evaluating this function returns what you entered at the promt.
...
;; Refine `greething' to say hello to everyone in `list-of-names':
(defun greeting ()
  (switch-to-buffer-other-window "*test*")
  (erase-buffer)
  (mapcar 'hello list-of-names)
...
;; You shold see all occurrences of "Hello" in the *test* buffer
;; replaced by "Bonjour".

;; You shold also get an error: "Search failed: Hello".
;;
